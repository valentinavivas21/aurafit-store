...
function productCardHTML(p) {
  ...
  const colorDots = p.colors.map(c =>
    `<span class="color-dot" style="background:${c.hex}" data-color="${c.name}" title="${c.name}"></span>`
  ).join('');
  return `
    <div class="product-card" style="cursor:pointer" onclick="openProductDetail('${p.id}')">
      <div style="height:280px;background:linear-gradient(135deg,${firstColor.hex}22,${firstColor.hex}44);display:flex;align-items:center;justify-content:center">
        <span style="font-family:var(--font-serif);font-size:48px;opacity:0.3;color:${firstColor.hex}">${p.name.charAt(0)}</span>
      </div>
      <div style="padding:20px">
        <div style="display:flex;gap:8px;margin-bottom:10px">${colorDots}</div>
  ...
  function renderDetail() {
    ...
    overlay.innerHTML = `
      <div class="modal-card" onclick="event.stopPropagation()" style="max-width:500px;width:90%">
        <button class="btn-ghost" style="float:right;font-size:20px" onclick="document.getElementById('product-detail-modal').remove()">×</button>
        <div style="height:200px;background:linear-gradient(135deg,${p.colors.find(c=>c.name===selectedColor).hex}22,${p.colors.find(c=>c.name===selectedColor).hex}55);display:flex;align-items:center;justify-content:center;margin:-32px -32px 24px">
        ...
        <div class="form-group">
          ...
          <div style="display:flex;gap:10px">${colorDots}</div>
        </div>
        <div class="form-group">
        ...
        ${selectedSize ? `
          <div class="form-group">
            ...
            <div style="display:flex;align-items:center;gap:12px">
              <button class="btn-ghost" onclick="event.stopPropagation();window._changeQty(-1)">−</button>
